...
def pulling_away(image):
    """Pulling Away (PT) Term for Avoding Mode Collapse"""
    batch_size = image.size(0)
    image = image.view(batch_size, -1)
    norm = image / ((image ** 2).sum()+1e-6).sqrt()
    normT = norm.transpose(1, 0)
    samples_similarity = (norm.matmul(normT).fill_diagonal_(0) ** 2) / (batch_size * (batch_size - 1))
    return samples_similarity.sum()
