...
def sample_images(generator, data_loader, fixed_noise, epoch):
    """Sample Images for Every Epoch"""

    with torch.no_grad():
        fake_celeba = generator.generate(fixed_noise)

        images, _ = next(iter(data_loader))
        images = images.to(device)
        recon_celeba = generator.reconstruct(images)

    save_image(denorm(fake_celeba[0:64].data),
               os.path.join(config.samples_gen_path, 'Face_Generation_Epoch_%03d.png' % (epoch+1))
               )

    save_image(denorm(recon_celeba[0:64].data),
               os.path.join(config.samples_recon_path, 'Face_Reconstruction_Epoch_%03d.png' % (epoch+1))
               )


def plot_losses(discriminator_losses, encoder_losses, decoder_losses, num_epochs, path):
    ...
    plt.plot(encoder_losses, label='Encoder', alpha=0.5)
    plt.plot(decoder_losses, label='Decoder', alpha=0.5)
    plt.legend(loc='best')
    plt.grid()
